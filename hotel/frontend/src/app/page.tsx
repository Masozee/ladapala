...
export default function Home() {
  ...
  // Transform weekly comparison data for dual bar chart
          ...
          <div className="p-4 bg-gray-50">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <button
                ...
                className="p-4 text-left bg-white hover:bg-gray-50 transition-colors rounded"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-6">
          ...
          <div className="bg-white border border-gray-200 md:col-span-2 lg:col-span-4">
            ...
            <div className="p-6 bg-gray-50">
              {(() => {
                const maxOccupation = Math.max(...occupationData.map(d => Math.max(d.currentMonth, d.previousMonth)));
                const scaleMax = getNiceMax(maxOccupation);
                    ...
                    <div className="flex-1 flex flex-col">
                      ...
                      <div className="flex-1 relative py-1">
                        ...
                        <div className="absolute inset-0 flex items-end justify-around gap-3 px-2">
                          {occupationData.map((day, index) => {
                            const prevBarHeight = scaleMax > 0 ? (day.previousMonth / scaleMax) * (chartHeight - 10) : 0;
                            const currBarHeight = scaleMax > 0 ? (day.currentMonth / scaleMax) * (chartHeight - 10) : 0;

                            return (
                              <div key={index} className="flex-1 flex items-end justify-center gap-1 max-w-[80px]">
                                {/* Previous Month Bar */}
                                <div className="flex-1 flex flex-col items-center justify-end group">
                                  ...
                                  <div className="opacity-0 group-hover:opacity-100 transition-opacity mb-2 bg-gray-900 text-white text-xs py-2 px-3 rounded shadow-lg absolute -translate-y-full whitespace-nowrap z-10">
                                    <div className="font-bold">{day.previousMonth} rooms</div>
                                    <div className="text-xs text-gray-300">Previous Month</div>
                                    <div className="text-xs text-gray-300">{formatDate(day.date)}</div>
                                  </div>

                                  {/* Bar */}
                                  <div
                                    className="w-full bg-gradient-to-t from-[#2baf6a] to-[#34d399] rounded-t transition-all duration-500 hover:opacity-80"
                                    style={{ height: `${prevBarHeight}px`, minHeight: '2px' }}
                                  ></div>
                                </div>
                                ...
                                <div className="flex-1 flex flex-col items-center justify-end group">
                                  {/* Tooltip */}
                                  <div className="opacity-0 group-hover:opacity-100 transition-opacity mb-2 bg-gray-900 text-white text-xs py-2 px-3 rounded shadow-lg absolute -translate-y-full whitespace-nowrap z-10">
                                    <div className="font-bold">{day.currentMonth} rooms</div>
                                  ...
                                  <div
                                    ...
                                    style={{ height: `${currBarHeight}px`, minHeight: '2px' }}
                                  ></div>
                                </div>
                              </div>
                            );
                      ...
                      {/* X-axis labels */}
                      <div className="flex justify-around gap-3 px-2 mt-2">
                        {occupationData.map((day, index) => (
                          <div key={index} className="flex-1 max-w-[80px] text-center">
                            <div className="text-[10px] font-medium text-gray-700">{day.day.substring(0, 3)}</div>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <div className="mt-4 space-y-3">
                <div className="flex items-center justify-center">
                  <div className="flex items-center space-x-6 text-xs">
                    <div className="flex items-center space-x-2">
                      <div className="w-3 h-3 bg-gradient-to-t from-[#2baf6a] to-[#34d399]"></div>
                      <span className="text-gray-600">Previous Month</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <div className="w-3 h-3 bg-gradient-to-t from-[#005357] to-[#007a7f]"></div>
                ...
                <div className="text-center px-6">
                  <p className="text-sm text-gray-600">
                    Comparison of room occupancy between the current month and the same week in the previous month.
                    This helps identify trends and performance changes in hotel operations.
                  </p>
                </div>
              </div>
            </div>
