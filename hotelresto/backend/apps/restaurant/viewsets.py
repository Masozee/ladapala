...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['post'], url_path='validate-room')
    def validate_room(self, request):
        """
        Validate hotel room for room charge payment
        - Checks if room number exists
        - Checks if guest is currently checked in
        ...
        try:
            # Import hotel models - they're in the same backend
            from apps.hotel.models import Room, CheckIn
            ...
            active_checkin = CheckIn.objects.filter(
                room=room,
                actual_checkout_time__isnull=True  # Not checked out yet
            ).select_related('reservation__guest').first()

            if not active_checkin:
                return Response(
                    {'error': f'No guest currently checked in to room {room_number}'},
                    status=status.HTTP_404_NOT_FOUND
                )
            ...
            return Response({
                ...
                'guest_id': guest.id,
                'reservation_id': active_checkin.reservation.id,
                'check_in_date': active_checkin.actual_checkin_time,
                'expected_checkout': active_checkin.reservation.checkout_date,
            })

        except Exception as e:
            return Response(
                {'error': f'Error validating room: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
