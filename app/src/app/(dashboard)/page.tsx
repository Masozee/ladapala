    ...
    <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {stats.map((stat) => {
          ...
          return (
            <Card key={stat.title} className="rounded-lg border-0">
          ...
          {unpaidTables.map((table) => (
            <Dialog key={table.tableNumber}>
              <DialogTrigger asChild>
                <div className="p-4 bg-orange-50 rounded-lg border border-orange-200 hover:bg-orange-100 cursor-pointer transition-colors">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {recentOrders.map((order) => (
              <Dialog key={order.id}>
                <DialogTrigger asChild>
                  <div className="p-4 bg-white rounded-lg border border-gray-200 hover:bg-gray-50 cursor-pointer transition-colors shadow-sm">
                      ...
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                          ...
                          {order.status === 'completed' ? <CheckCircle className="h-5 w-5 text-green-600" /> :
                           order.status === 'preparing' ? <ChefHat className="h-5 w-5 text-blue-600" /> :
                           order.status === 'ready' ? <Utensils className="h-5 w-5 text-yellow-600" /> :
