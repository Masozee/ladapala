...
class GuestSerializer(serializers.ModelSerializer):
    ...
    def get_average_rating(self, obj):
        """Calculate average rating from checked-out reservations with reviews"""
        from ..models import Reservation

        # Get all checked-out reservations with ratings
        reservations_with_ratings = Reservation.objects.filter(
            guest=obj,
            status='CHECKED_OUT',
            guest_rating__isnull=False
        )

        if not reservations_with_ratings.exists():
            return None

        # Calculate average
        total_rating = sum(res.guest_rating for res in reservations_with_ratings)
        count = reservations_with_ratings.count()
    ...
    def get_reservations(self, obj):
        ...
        return [{
            ...
            'guest_rating': res.guest_rating,
            'guest_review': res.guest_review,
...
class ReservationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'can_cancel', 'total_rooms',
            'guest_rating', 'guest_review', 'reviewed_at',  # Rating fields
            'created_at', 'updated_at'
        ]
        read_only_fields = ['created_at', 'updated_at', 'reservation_number', 'nights', 'reviewed_at']
...
class PaymentSerializer(serializers.ModelSerializer):
    ...
    # Promotion fields - using SerializerMethodField for computed values
    ...
    loyalty_points_redeemed = serializers.SerializerMethodField()
    loyalty_points_value = serializers.SerializerMethodField()
    loyalty_points_earned = serializers.SerializerMethodField()
    subtotal = serializers.SerializerMethodField()
    total_discount = serializers.SerializerMethodField()

    def get_voucher(self, obj):
        """Get voucher ID if voucher was used"""
        usage = obj.voucher_usage.first() if hasattr(obj, 'voucher_usage') else None
        return usage.voucher.id if usage else None

    def get_voucher_code(self, obj):
        """Get voucher code if voucher was used"""
        usage = obj.voucher_usage.first() if hasattr(obj, 'voucher_usage') else None
        return usage.voucher.code if usage else None

    def get_voucher_name(self, obj):
        """Get voucher name if voucher was used"""
        usage = obj.voucher_usage.first() if hasattr(obj, 'voucher_usage') else None
        return usage.voucher.name if usage else None

    def get_voucher_discount(self, obj):
        """Get voucher discount amount"""
        usage = obj.voucher_usage.first() if hasattr(obj, 'voucher_usage') else None
        return float(usage.discount_amount) if usage else 0.0

    def get_discount(self, obj):
        """Get discount ID - not implemented yet"""
        return None

    def get_discount_name(self, obj):
        """Get discount name - not implemented yet"""
        return None

    def get_discount_type(self, obj):
        """Get discount type - not implemented yet"""
        return None

    def get_discount_amount(self, obj):
        """Get discount amount - not implemented yet"""
        return 0.0

    def get_loyalty_points_redeemed(self, obj):
        """Get loyalty points redeemed - not implemented yet"""
        return 0

    def get_loyalty_points_value(self, obj):
        """Get loyalty points value - not implemented yet"""
        return 0.0

    def get_loyalty_points_earned(self, obj):
        """Get loyalty points earned - not implemented yet"""
        return 0

    def get_subtotal(self, obj):
        """Get subtotal (amount before discounts)"""
        return float(obj.amount)

    def get_total_discount(self, obj):
        """Calculate total discount from all sources"""
        voucher_discount = self.get_voucher_discount(obj)
        discount_amount = self.get_discount_amount(obj)
        loyalty_value = self.get_loyalty_points_value(obj)
        return voucher_discount + discount_amount + loyalty_value
...
class GuestListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'full_name', 'email', 'phone', 'nationality', 'is_vip',
                  ...
                  'total_stays', 'total_spent', 'average_rating']
